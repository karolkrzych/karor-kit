...
1. Evidence, not philosophy. Sources by value: **Retro inbox** — Linear team doc "[kit] Retro inbox" (https://linear.app/karor/document/kit-retro-inbox-c1a7f2895bc9; fetch via MCP, running friction log) -> user's own annoyance list (ask first, plainly) -> recent PRs (what did humans fix AFTER agent finished?) -> implementer reports (recurring Surprises/Open) -> bounced tickets.
