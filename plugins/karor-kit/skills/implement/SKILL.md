...
5. Verdict:
   - OK -> PR per repo convention, issue -> In Review; merge + push yourself (default since 2026-08-29, unless told otherwise) -> Done, then **worktree teardown** (see Rules) -> post-merge comment per template + append `## Cost` dispatch lines to issue body AT Done (format in template): one line per dispatch, model + tokens; the `(orchestration)` line (fable) is appended at session close-out.
