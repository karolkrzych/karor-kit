...
4. **Land wave** before next starts: merge PRs locally into main, commit message `Merge pull request #N from <owner>/<branch>` — GitHub marks PRs MERGED once main lands. `gh pr merge` = classifier-blocked, don't use. After merges: `--import` step (global `class_name` set changed) + full suite; red -> stop batch, report. Push main ONLY with explicit authorization in the batch invocation; absent or blocked -> hand back: local main ready, human pushes.
...
6. **Close out**: per ticket Done + `## Cost` per `implement`; orchestrator tokens split evenly across batch tickets (`(orchestration, batch/N)`). Batch NOT closed without human playtest smoke — human plays, feedback -> input for `refinement`. Report: landed tickets, suite result, refusals/flakes, playtest ask.
