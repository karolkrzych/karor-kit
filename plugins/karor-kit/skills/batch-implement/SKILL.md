...
6. **Close out**: per ticket Done + `## Cost` dispatch lines IMMEDIATELY at that ticket's Done (wave landing), per `implement`; orchestrator tokens split evenly across batch tickets (`(orchestration, batch/N)`), appended to EVERY batch ticket at batch close-out. **Batch note** -> project's Linear doc `[<project>] Batch log` (create if missing, newest section on top): dated `##` header, ticket table (id + title), "co weszło" summary grouped by theme, end state (main hash, suite result, PR range). Batch NOT closed without human playtest smoke — human plays, feedback -> input for `refinement`. Report: landed tickets, suite result, refusals/flakes, playtest ask.
