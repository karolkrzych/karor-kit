...
4. Explicit approval ("push 1, 3") -> expand approved to full template (incl. priority per build order). Then surface EVERY baked-in assumption (placeholder values, look/visuals, mechanics details) as a numbered question list with recommended defaults — user answers any subset; "skip" accepts all defaults. No silent assumptions. Player-facing ticket -> `## Player-visible outcome` filled (what player SEES/FEELS) or asked as one of the questions; unnameable = gap. THEN create via MCP. Complete -> Todo; gapped -> Backlog.
